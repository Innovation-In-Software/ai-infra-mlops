#!/usr/bin/env python3
"""Copy canonical Lab 10 screenshots from instructor captures to lab10/images/."""
...
MANIFEST = REPO / "lab10" / "config" / "lab10_screenshot_manifest.json"
IMAGES = REPO / "lab10" / "images"
DEFAULT_SOURCE = Path(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Lab 10 screenshots to lab10/images/")
    ...
    for dest_name, rel_src in data.get("reuse_from_lab9", {}).items():
        src = REPO / rel_src
        if src.is_file():
            shutil.copy2(src, IMAGES / dest_name)
            print(f"   REUSE lab9: {rel_src} -> {dest_name}")
            copied += 1

    allowed = (
        set(data.get("canonical", {}).values())
        | set(data.get("aliases", {}).keys())
        | set(data.get("reuse_from_lab9", {}).keys())
    )
