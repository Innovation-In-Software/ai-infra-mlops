#!/usr/bin/env python3
"""Copy canonical Lab 9 screenshots from instructor captures to lab9/images/."""
...
MANIFEST = REPO / "lab9" / "config" / "lab9_screenshot_manifest.json"
IMAGES = REPO / "lab9" / "images"
DEFAULT_SOURCE = Path(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Lab 9 screenshots to lab9/images/")
    ...
    for dest_name, rel_src in data.get("reuse_from_lab8", {}).items():
        src = REPO / rel_src
        if src.is_file():
            shutil.copy2(src, IMAGES / dest_name)
            print(f"   REUSE lab8: {rel_src} -> {dest_name}")
            copied += 1

    allowed = (
        set(data.get("canonical", {}).values())
        | set(data.get("aliases", {}).keys())
        | set(data.get("reuse_from_lab8", {}).keys())
    )
