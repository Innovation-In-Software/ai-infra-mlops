#!/usr/bin/env python3
"""Copy canonical Lab 5 screenshots from instructor captures to lab5/images/."""
...
MANIFEST = REPO / "lab5" / "config" / "lab5_screenshot_manifest.json"
IMAGES = REPO / "lab5" / "images"
DEFAULT_SOURCE = Path(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Lab 5 screenshots to lab5/images/")
    ...
    for dest_name, rel_src in data.get("reuse_from_lab4", {}).items():
        src = REPO / rel_src
        if src.is_file():
            shutil.copy2(src, IMAGES / dest_name)
            print(f"   REUSE lab4: {rel_src} -> {dest_name}")

    allowed = (
        set(data.get("canonical", {}).values())
        | set(data.get("aliases", {}).keys())
        | set(data.get("reuse_from_lab4", {}).keys())
    )
    ...
    if args.git_push:
        subprocess.run(
            [
                ...
                "add",
                "lab5/images/",
                "lab5/config/lab5_screenshot_manifest.json",
                "lab5/STEPS.md",
                "scripts/sync_lab5_screenshots.py",
            ],
            cwd=REPO,
            check=False,
        )
        subprocess.run(
            ["git", "commit", "-m", "Lab 5: sync participant screenshots and update STEPS."],
            cwd=REPO,
            check=False,
        )
        subprocess.run(["git", "push", "origin", "main"], cwd=REPO, check=False)

    return 0 if copied or data.get("reuse_from_lab4") else 1
