#!/usr/bin/env python3
"""Copy canonical Lab 6 screenshots from instructor captures to lab6/images/."""
...
MANIFEST = REPO / "lab6" / "config" / "lab6_screenshot_manifest.json"
IMAGES = REPO / "lab6" / "images"
DEFAULT_SOURCE = Path(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Lab 6 screenshots to lab6/images/")
    ...
    for dest_name, rel_src in data.get("reuse_from_lab5", {}).items():
        src = REPO / rel_src
        if src.is_file():
            shutil.copy2(src, IMAGES / dest_name)
            print(f"   REUSE lab5: {rel_src} -> {dest_name}")
            copied += 1

    allowed = (
        set(data.get("canonical", {}).values())
        | set(data.get("aliases", {}).keys())
        | set(data.get("reuse_from_lab5", {}).keys())
    )
