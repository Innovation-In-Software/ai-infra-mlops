#!/usr/bin/env python3
"""Copy canonical Lab 8 screenshots from instructor captures to lab8/images/."""
...
MANIFEST = REPO / "lab8" / "config" / "lab8_screenshot_manifest.json"
IMAGES = REPO / "lab8" / "images"
DEFAULT_SOURCE = Path(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Lab 8 screenshots to lab8/images/")
    ...
    for dest_name, rel_src in data.get("reuse_from_lab7", {}).items():
        src = REPO / rel_src
        if src.is_file():
            shutil.copy2(src, IMAGES / dest_name)
            print(f"   REUSE lab7: {rel_src} -> {dest_name}")
            copied += 1

    allowed = (
        set(data.get("canonical", {}).values())
        | set(data.get("aliases", {}).keys())
        | set(data.get("reuse_from_lab7", {}).keys())
    )
